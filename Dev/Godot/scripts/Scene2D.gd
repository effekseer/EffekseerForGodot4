...
func _ready():
	...
	$GUI/EffectMenu.connect("effect_choosed",Callable(self,"_on_effect_choosed"))
	$GUI/PlayButton.connect("pressed",Callable(self,"_on_play_button_pressed"))
	$GUI/StopButton.connect("pressed",Callable(self,"_on_stop_button_pressed"))
	$GUI/PauseButton.connect("pressed",Callable(self,"_on_pause_button_pressed"))
	for i in range(4):
		$GUI/Triggers/Buttons.get_child(i).connect("pressed",Callable(self,"_on_trigger_button_pressed").bind(i))
