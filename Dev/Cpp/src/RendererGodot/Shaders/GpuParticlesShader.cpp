...
static const char src_particle_process_uniforms[] = R"(
...
static const char src_particle_process_functions[] = R"(
...
static const char src_particle_vortex[] = R"(
...
static const char src_particle_start_head[] = R"(
...
static const char src_particle_start_tail[] = R"(
...
static const char src_particle_start_emit_line[] = R"(
...
static const char src_particle_start_emit_circle[] = R"(
...
static const char src_particle_start_emit_sphere[] = R"(
...
static const char src_particle_start_emit_model[] = R"(
...
static const char src_particle_process_head[] = R"(
...
static const char src_particle_process_tail[] = R"(
...
static const char src_particle_process_gravity[] = R"(
	VELOCITY += paramGravity * DELTA;
)";

static const char src_particle_process_damping[] = R"(
...
static const char src_particle_process_apply_position[] = R"(
	position += VELOCITY * DELTA;
)";

static const char src_particle_process_vortex[] = R"(
...
static const char src_particle_process_turbulence[] = R"(
...
static const char src_particle_process_update_direction[] = R"(
...
static const char src_particle_process_rotation[] = R"(
...
static const char src_particle_process_scale_fixed[] = R"(
	vec4 scale = RandomFloat4Range(seed, paramScaleData0, paramScaleData1);
)";

static const char src_particle_process_scale_easing[] = R"(
	...
	vec4 scale = mix(scale1, scale2, EasingSpeed(lifeRatio, paramScaleEasing));
)";

static const char src_particle_process_color_fixed[] = R"(
	vec4 color = UnpackColor(paramColorData.x);
)";

static const char src_particle_process_color_random[] = R"(
	vec4 color = RandomColorRange(seed, paramColorData.x, paramColorData.y);
)";

static const char src_particle_process_color_easing[] = R"(
	...
	vec4 color = mix(colorStart, colorEnd, EasingSpeed(lifeRatio, paramColorEasing));
)";

static const char src_particle_process_color_gradient[] = R"(
	vec4 color = texture(gradientTexture, vec2(lifeRatio, 0.0));
)";

static const char src_particle_process_color_hsv[] = R"(
	color.rgb = HSV2RGB(color.rgb);
)";

static const char src_particle_process_color_inherit_always[] = R"(
	color *= UnpackColor(emitterColor);
)";

static const char src_particle_process_color_inherit_when_created[] = R"(
	color *= UnpackColor(floatBitsToUint(USERDATA1.y));
)";

static const char src_particle_process_emissive[] = R"(
	color.rgb *= paramEmissive;
)";

static const char src_particle_process_fade_in[] = R"(
	color.a *= clamp(lifeAge / paramFadeIn, 0.0, 1.0);
)";

static const char src_particle_process_fade_out[] = R"(
...
godot::RID GpuParticlesShader::GenerateProcessShader(bool glcompatibleMode, const Effekseer::GpuParticles::ParamSet& paramSet)
{
	...
	code += src_particle_process_uniforms;
	...
	code += src_particle_process_functions;

	code += "void start() {\n";
	code += src_particle_start_head;

	switch (paramSet.EmitShape.Type)
	{
	case EmitShapeT::Line:
		code += src_particle_start_emit_line;
		break;
	case EmitShapeT::Circle:
		code += src_particle_start_emit_circle;
		break;
	case EmitShapeT::Sphere:
		code += src_particle_start_emit_sphere;
		break;
	case EmitShapeT::Model:
		code += src_particle_start_emit_model;
		break;
	}

	code += src_particle_start_tail;
	code += "}\n";

	if (paramSet.Force.VortexRotation != 0.0f || paramSet.Force.VortexAttraction != 0.0f) {
		code += src_particle_vortex;
	}

	code += "void process() {\n";
	code += src_particle_process_head;

	if (!Vec3f::Load(&paramSet.Force.Gravity).IsZero()) {
		code += src_particle_process_gravity;
	}

	if (!Vec2f::Load(&paramSet.Velocity.Damping).IsZero()) {
		code += src_particle_process_damping;
	}

	code += src_particle_process_apply_position;

	if (paramSet.Force.VortexRotation != 0.0f || paramSet.Force.VortexAttraction != 0.0f) {
		code += src_particle_process_vortex;
	}

	if (paramSet.Force.TurbulencePower != 0.0f) {
		code += src_particle_process_turbulence;
	}

	code += src_particle_process_update_direction;
	
	code += src_particle_process_rotation;

	switch (paramSet.Scale.Type) {
	case ScaleType::Fixed:
		code += src_particle_process_scale_fixed;
		break;
	case ScaleType::Easing:
		code += src_particle_process_scale_easing;
		break;
	}

	switch (paramSet.RenderColor.ColorAllType) {
	case ColorParamType::Fixed:
		code += src_particle_process_color_fixed;
		break;
	case ColorParamType::Random:
		code += src_particle_process_color_random;
		break;
	...
	case ColorParamType::Gradient:
		code += src_particle_process_color_gradient;
		break;
	}

	if (paramSet.RenderColor.ColorSpace == ColorSpaceType::HSVA) {
		code += src_particle_process_color_hsv;
	}

	if (paramSet.RenderColor.ColorInherit == Effekseer::BindType::Always ||
		paramSet.RenderColor.ColorInherit == Effekseer::BindType::NotBind_Root) {
		code += src_particle_process_color_inherit_always;
	}
	else {
		code += src_particle_process_color_inherit_when_created;
	}
	
	code += src_particle_process_emissive;

	if (paramSet.RenderColor.FadeIn > 0.0f) {
		code += src_particle_process_fade_in;
	}
	if (paramSet.RenderColor.FadeOut > 0.0f) {
		code += src_particle_process_fade_out;
	}

	code += src_particle_process_tail;
	code += "}\n";
