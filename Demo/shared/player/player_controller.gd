...
@export var first_jump_force := 7.0
...
@onready var _shot_fireball := load("res://shared/player/fireball.tscn")
...
func spawn_shot():
	var instance = _shot_fireball.instantiate()
	instance.rotation = _rotation_root.global_rotation
	instance.position = global_position + Vector3(0, 0.5, 0)
...
func _physics_process(delta: float) -> void:
	...
	var is_just_jumping := Input.is_action_just_pressed("act_jump") and is_on_floor()
	var is_air_boosting := Input.is_action_pressed("act_jump") and not is_on_floor() and y_velocity > 0.0
	...
	if _is_animating:
		...
		if is_on_floor():
			if _frames_on_floor < floor_inertia_frames:
				...
				var angle_to_clamped := angle_to_move #clampf(angle_to_move, -turning_speed * delta, turning_speed * delta)
...
func _get_camera_oriented_input() -> Vector3:
	var raw_input := Input.get_vector("act_move_left", "act_move_right", "act_move_up", "act_move_down")
